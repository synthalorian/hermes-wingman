use crate::helpers::{classify_provider, read_config, read_file};
use crate::platform::{find_hermes_binary, hermes_binary_path, run_hermes};
...
"##;
    ...
    let output = Command::new("python3")
        .args([
            py_script.to_str().unwrap_or("/tmp/save_env.py"),
            "set",
            key,
            value,
        ])
...
pub async fn gateway_get_platforms(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    let gateway_state = read_file(&gw_path)
        .ok()
    ...
    for mut platform in definitions {
        ...
        let runtime_status = gateway_state
            .as_ref()
        ...
        let status = if has_token {
            if runtime_status == "connected" {
                "connected"
            } else if runtime_status == "retrying" || runtime_status == "error" {
                "error"
            } else {
                "configured"
            }
        ...
        if let Some(vars) = platform["vars"].as_array_mut() {
            for var in vars.iter_mut() {
                ...
                if !current.is_empty() {
                    var["current"] = serde_json::json!(if is_password {
                        if current.len() > 8 {
...
pub async fn gateway_configure_platform(
    ...
    let vars = match vars {
        Some(v) => v,
        None => {
            return Json(serde_json::json!({"success": false, "error": "vars object required"}))
        }
    };
...
pub async fn gateway_service_action(Path(action): Path<String>) -> Json<serde_json::Value> {
    let hermes_args: &[&str] = match action.as_str() {
        ...
        _ => {
            return Json(
                serde_json::json!({"success": false, "error": format!("Unknown action: {}", action)}),
            )
        }
    };
...
pub async fn get_gateway(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    match read_file(&gw_path) {
        Ok(content) => match serde_json::from_str::<serde_json::Value>(&content) {
            ...
            Err(e) => {
                Json(serde_json::json!({"running": false, "platforms": [], "error": e.to_string()}))
            }
        },
    ...
    match action {
        "start" => {
            ...
            match run_hermes(&["gateway", "start"]) {
                ...
                Ok((_, stderr, _)) => Json(
                    serde_json::json!({"success": false, "action": "start", "error": stderr.trim()}),
                ),
                Err(e) => {
                    Json(serde_json::json!({"success": false, "action": "start", "error": e}))
                }
            }
        }
        "stop" => {
                ...
                Ok(mut child) => {
                    ...
                    match child.wait_with_output() {
                        Ok(output) if output.status.success() => Json(
                            serde_json::json!({"success": true, "action": "stop", "running": false}),
                        ),
                        Ok(output) => {
                            let stderr = String::from_utf8_lossy(&output.stderr).to_string();
                            Json(
                                serde_json::json!({"success": false, "action": "stop", "error": stderr.trim()}),
                            )
                        }
                        Err(e) => Json(
                            serde_json::json!({"success": false, "action": "stop", "error": e.to_string()}),
                        ),
                    }
                }
                Err(e) => Json(
...
pub async fn detect_setup(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    let has_api_keys = config["providers"]
        ...
        .map(|m| {
            m.values()
                .any(|v| v["api_key"].is_string() || v["api_key_env"].is_string())
        })
        .unwrap_or(false);
        ...
        "brew" => {
                ...
                .output()
            {
                Ok(output) if output.status.success() => {
                    ...
                    Json(
                        serde_json::json!({"success": true, "output": out.chars().take(500).collect::<String>()}),
                    )
                }
                Ok(output) => {
                    let stderr = String::from_utf8_lossy(&output.stderr).to_string();
                    Json(
                        serde_json::json!({"success": false, "error": stderr.chars().take(500).collect::<String>()}),
                    )
                }
                Err(e) => Json(
                    serde_json::json!({"success": false, "error": format!("Homebrew not found: {}", e)}),
                ),
            }
        }
        "pip" => {
            ...
            for pip_cmd in &["pip3", "pip"] {
                    ...
                    .output()
                {
                    ...
                    if err_lower.contains("externally-managed-environment")
                        || err_lower.contains("externally managed")
                    {
