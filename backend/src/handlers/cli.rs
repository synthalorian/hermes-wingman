...
pub async fn hermes_command(Json(body): Json<HermesCommandBody>) -> Json<serde_json::Value> {
...
pub async fn cli_fallback_add(Json(body): Json<serde_json::Value>) -> Json<serde_json::Value> {
    ...
    if provider.is_empty() || model.is_empty() {
        return Json(
            serde_json::json!({ "success": false, "error": "provider and model required" }),
        );
    }
    match run_hermes(&["fallback", "add", "--provider", provider, "--model", model]) {
        Ok((stdout, _, code)) => {
            Json(serde_json::json!({ "success": code == 0, "stdout": stdout.trim() }))
        }
        Err(e) => Json(serde_json::json!({ "success": false, "error": e })),
    }
}
...
pub async fn cli_webhook_list() -> Json<serde_json::Value> {
    match run_hermes(&["webhook", "list"]) {
        Ok((stdout, _, _)) => {
            Json(serde_json::json!({ "success": true, "webhooks": stdout.trim() }))
        }
        Err(e) => Json(serde_json::json!({ "success": false, "error": e })),
    }
}
...
pub async fn cli_plugins_list() -> Json<serde_json::Value> {
    match run_hermes(&["plugins", "list"]) {
        Ok((stdout, _, _)) => {
            Json(serde_json::json!({ "success": true, "plugins": stdout.trim() }))
        }
        Err(e) => Json(serde_json::json!({ "success": false, "error": e })),
    }
}
...
pub async fn cli_mcp_list() -> Json<serde_json::Value> {
    match run_hermes(&["mcp", "list"]) {
        Ok((stdout, _, _)) => {
            Json(serde_json::json!({ "success": true, "servers": stdout.trim() }))
        }
        Err(e) => Json(serde_json::json!({ "success": false, "error": e })),
    }
}
...
pub async fn cli_doctor() -> Json<serde_json::Value> {
    match run_hermes(&["doctor"]) {
        Ok((stdout, _, code)) => {
            Json(serde_json::json!({ "success": code == 0, "output": stdout.trim() }))
        }
        Err(e) => Json(serde_json::json!({ "success": false, "error": e })),
    }
}
...
pub async fn cli_insights() -> Json<serde_json::Value> {
    match run_hermes(&["insights", "--days", "7"]) {
        Ok((stdout, _, _)) => {
            Json(serde_json::json!({ "success": true, "insights": stdout.trim() }))
        }
        Err(e) => Json(serde_json::json!({ "success": false, "error": e })),
    }
}
