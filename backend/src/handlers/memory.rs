...
pub async fn memory_list() -> Json<serde_json::Value> {
    ...
    for section in memory_content.split('§') {
        ...
        let key = if first_line.starts_with('#') {
            first_line.trim_start_matches('#').trim().to_string()
        } else if first_line.starts_with("**") {
            first_line.trim_matches('*').to_string()
        } else {
...
pub async fn memory_delete(
    ...
    match run_hermes(&["memory", "delete", &id]) {
        Ok((_stdout, _stderr, code)) => Json(serde_json::json!({
            "success": code == 0,
            "id": id,
        })),
        Err(e) => Json(serde_json::json!({
...
pub async fn memory_search(Json(body): Json<serde_json::Value>) -> Json<serde_json::Value> {
